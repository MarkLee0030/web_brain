...
export function normalizeDownloadDirectory(value) {
  ...
  // Windows absolute paths (e.g. D:/images) are accepted so extension
  // downloads can land outside the Downloads folder; onDeterminingFilename
  // routes absolute filenames as-is. Drive-relative paths (C:foo) stay invalid.
  const absolute = raw.match(/^([a-zA-Z]:)(\/.*)$/);
  if (absolute) {
    const parts = absolute[2].split('/').map((part) => part.trim());
    if (parts.some((part) => !part || part === '.' || part === '..')) return '';
    if (parts.some((part) => /[\u0000-\u001f\u007f<>:"|?*]/.test(part))) return '';
    return `${absolute[1]}/${parts.join('/')}`;
  }
