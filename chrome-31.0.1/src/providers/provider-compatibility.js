...
export function detectedCompatibilityPreset(config = {}) {
  ...
  if (model.includes('muse') || model.includes('glimmer')) return 'muse';
...
export function compatibilityRequestBody(config = {}) {
  ...
  if (preset === 'qwen') {
    // Qwen3.5-class and newer templates expose reasoning_effort levels
    // (low/medium/xhigh); older Qwen3 templates only have the binary
    // enable_thinking switch.
    if (/qwen3[._-]?[5-9]/.test(clean(config.model))) {
      if (!enabled) return { chat_template_kwargs: { enable_thinking: false } };
      const level = {
        minimal: 'low',
        low: 'low',
        medium: 'medium',
      ...
      if (level) return { chat_template_kwargs: { reasoning_effort: level, preserve_thinking: true } };
    }
    return {
  ...
  if (preset === 'muse') {
    // Muse-Glimmer reasoning cannot be disabled; reasoning_strength is the
    // trained ladder low/medium/high/xhigh. off/minimal clamp to low.
    const level = {
      off: 'low',
      minimal: 'low',
      low: 'low',
    ...
    if (level) return { chat_template_kwargs: { reasoning_strength: level } };
    return {};
  }
  if (preset === 'deepseek') {
...
export function mergeProviderRequestBody(body, config = {}, perRequestExtraBody = undefined) {
  ...
  if (extras.chat_template_kwargs?.enable_thinking === false) {
    ...
    delete extras.chat_template_kwargs.reasoning_effort;
  }
