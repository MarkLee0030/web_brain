...
const thinkingStrengthBtn = document.getElementById('thinking-strength-btn');
...
chrome.storage.onChanged.addListener((changes) => {
  ...
  if (changes.workingDirectory) {
    workingDirState = changes.workingDirectory.newValue || null;
...
async function resetChatHistoryStateForTab(tabId) {
  ...
  // Starting a fresh conversation must not erase the old conversation from
  // the history view. Drop only the in-memory identity so the next save
  // creates a NEW history record; the previous record stays in IndexedDB.
...
async function loadProviders() {
  try {
    ...
    providersById = new Map(Object.entries(res.providers || {}));
...
// A compact header button that cycles the active provider's reasoning level.
// Values map onto the existing compat.reasoningEffort channel so the next
// request picks the new level up with no reload (see compatibilityRequestBody
// for the per-model mapping: muse → reasoning_strength low/medium/high/xhigh,
// Qwen3.5+ → reasoning_effort low/medium/xhigh + off).
let providersById = new Map();

const THINKING_LADDERS = {
  muse: ['auto', 'low', 'medium', 'high', 'xhigh'],
  qwen: ['auto', 'off', 'low', 'medium', 'xhigh'],
};
const THINKING_LEVEL_FALLBACK = {
  muse: { off: 'low', minimal: 'low', max: 'xhigh' },
  qwen: { minimal: 'low', high: 'xhigh', max: 'xhigh' },
};

function thinkingFamilyForModel(model) {
  const name = String(model || '').toLowerCase();
  if (/muse|glimmer/.test(name)) return 'muse';
  if (/qwen3[._-]?[5-9]/.test(name)) return 'qwen';
  return null;
}

function normalizeThinkingLevel(family, value) {
  const ladder = THINKING_LADDERS[family];
  let level = value || 'auto';
  if (!ladder.includes(level)) {
    level = THINKING_LEVEL_FALLBACK[family]?.[level] || 'auto';
  }
  return ladder.includes(level) ? level : 'auto';
}

function syncThinkingStrengthButton() {
  if (!thinkingStrengthBtn || !thinkingStrengthLabel) return;
  const config = providersById.get(providerSelect?.value);
  const family = config ? thinkingFamilyForModel(config.model) : null;
  if (!family) {
    thinkingStrengthBtn.classList.add('hidden');
    return;
  }
  thinkingStrengthBtn.classList.remove('hidden');
  const level = normalizeThinkingLevel(
    family,
    config.compat?.reasoningEffort ?? config.reasoningEffort,
  );
  thinkingStrengthBtn.dataset.family = family;
  thinkingStrengthBtn.dataset.level = level;
  thinkingStrengthLabel.textContent = `${t('sp.thinking.prefix')}${level}`;
  thinkingStrengthBtn.setAttribute('aria-label', t('sp.thinking.title', { level }));
}

thinkingStrengthBtn?.addEventListener('click', async () => {
  const id = providerSelect?.value;
  const config = providersById.get(id);
  const family = thinkingStrengthBtn?.dataset.family;
  if (!id || !config || !family || !THINKING_LADDERS[family]) return;
  const ladder = THINKING_LADDERS[family];
  const current = normalizeThinkingLevel(
    family,
    thinkingStrengthBtn.dataset.level || config.compat?.reasoningEffort || config.reasoningEffort,
  );
  const next = ladder[(ladder.indexOf(current) + 1) % ladder.length];
  const merged = {
    ...config,
    compat: { ...(config.compat || {}), reasoningEffort: next },
  };
  try {
    await sendToBackground('update_provider', {
      providerId: id,
    ...
    console.warn('[WebBrain] failed to update thinking strength:', e);
    return;
  }
  providersById.set(id, merged);
  syncThinkingStrengthButton();
});

// ─── Working directory (sandboxed filesystem) ────────────────────────────
// Header button that picks the folder the workspace_* tools are sandboxed to.
// showDirectoryPicker must run here (window context + user gesture); the
// resulting handle is posted to the background, which persists it in
// IndexedDB and mirrors the name to chrome.storage.local. Picking (or
// re-picking) also re-grants the readwrite permission that Chrome revokes
// across browser restarts.
let workingDirState = null;
let workingDirPermission = 'none';

function syncWorkingDirButton() {
  if (!workingDirBtn || !workingDirLabel) return;
  const name = workingDirState?.name || '';
  const needsAuth = !!name && workingDirPermission !== 'granted';
  workingDirLabel.textContent = name
    ? (needsAuth ? `${name} ⚠` : name)
    : t('sp.working_dir.none');
  workingDirBtn.title = name
    ? t(needsAuth ? 'sp.working_dir.needs_auth' : 'sp.working_dir.title')
    : t('sp.working_dir.title');
}

async function loadWorkingDirState() {
  try {
    const res = await sendToBackground('get_working_directory_state');
    workingDirState = res?.workingDirectory || null;
    workingDirPermission = res?.permission || 'none';
  } catch { /* keep defaults */ }
  syncWorkingDirButton();
}
...
workingDirBtn?.addEventListener('click', async () => {
  try {
    ...
    if (e?.name !== 'AbortError') {
      console.warn('[WebBrain] working directory pick failed:', e);
    }
  }
});
