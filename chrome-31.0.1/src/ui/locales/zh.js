...
export default {
  ...
  'sp.btn.language': '语言',
  'sp.thinking.prefix': '思考:',
  'sp.thinking.title': '思考强度：{level} — 点击循环切换',
  ...
  'st.persona.heading': '助手人设',
  'st.persona.desc_html': '自定义代理的身份与行为规则。每次对话（所有模式）都会把它<strong>紧跟在基础提示词之后</strong>追加进系统提示。留空 = 不注入。',
  'st.persona.text.label': '人设文本',
  'st.persona.text.placeholder': '你的名字叫「私人AI网页助手」，运行在用户自己的浏览器里。\n- 始终用中文回答，先给结论，再给依据，简洁不啰嗦。\n- 语气自然，像靠谱的同事。',
  'st.persona.save': '保存',
  'st.persona.clear': '清除',
  'st.persona.saved': '已保存。',
  'st.persona.cleared': '已清除。',
  ...
  "st.display.download_directory.desc": "模型触发下载的保存位置：可以是浏览器/系统“下载”目录下的相对子文件夹，也可以是绝对路径（如 D:/images，文件夹需已存在）。留空使用系统默认下载目录。",
  "st.display.download_directory.placeholder": "系统默认值",
  "st.display.download_directory.error": "请使用 WebBrain 或 Work/WebBrain 这样的相对文件夹，或 D:/images 这样的绝对路径（仅 Windows 盘符路径）。不允许使用“..”。",
  "st.display.workspace_whitelist.label": "工作目录白名单",
  "st.display.workspace_whitelist.desc_html": "额外授权给代理的本地文件夹：代理可以<b>读取</b>这些目录并把文件<b>搬移进</b>工作目录（workspace_whitelist_list / workspace_copy_in）。典型用途：有反爬拦截的网站只能靠虚拟触控点击下载按钮，文件会落到 Chrome 默认下载文件夹——把「下载」文件夹加进来，代理就能自动把文件搬进工作目录。<br>提示：浏览器重启后需要点击「重新授权」。",
  "st.display.workspace_whitelist.add": "添加目录",
  ...
  'st.skills.saved': '修改已保存。',
  'st.skills.editing': '正在编辑：{name}——修改后点击下方「保存修改」。',
