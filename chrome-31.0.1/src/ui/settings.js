...
const personaTestResult = document.getElementById('test-persona');
...
async function init() {
  ...
  const profileStored = await chrome.storage.local.get(['profileEnabled', 'profileText', 'personaText']);
  ...
  if (personaTextArea) personaTextArea.value = profileStored.personaText || '';
...
async function reloadProfileSyncData() {
  ...
  if (personaTextArea) personaTextArea.value = stored.personaText || '';
...
if (btnSavePersona) {
  btnSavePersona.addEventListener('click', async () => {
    const text = (personaTextArea?.value || '').trim();
    await chrome.storage.local.set({ personaText: text });
    flashPersonaResult('ok', t('st.persona.saved'));
  });
}

if (btnClearPersona) {
  btnClearPersona.addEventListener('click', async () => {
    if (personaTextArea) personaTextArea.value = '';
    await chrome.storage.local.set({ personaText: '' });
    flashPersonaResult('ok', t('st.persona.cleared'));
  });
}

function flashPersonaResult(className, text) {
  if (!personaTestResult) return;
  personaTestResult.className = `test-result show ${className}`;
  personaTestResult.textContent = text;
  setTimeout(() => personaTestResult.classList.remove('show'), 2000);
}
