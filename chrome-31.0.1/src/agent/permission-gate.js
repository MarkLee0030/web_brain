...
export const Capability = {
  ...
  FILESYSTEM: 'filesystem',      // workspace_* mutations inside the user-picked working directory
};
...
export const CAPABILITY_LABEL = {
  ...
  [Capability.FILESYSTEM]: 'modify files and folders in',
};
...
const TOOL_CAPABILITY = {
  ...
  // Working-directory mutations. Sandboxed by the FileSystemDirectoryHandle
  // root + normalizeWorkspacePath (see workspace-fs.js); the FILESYSTEM grant
  // is the user-facing consent for modifying the picked folder.
...
export function capabilitiesFor(name, args) {
  ...
  if (name === 'workspace_download') {
    // Writing into the folder needs a FILESYSTEM grant AND fetching from each
    // URL host needs the per-host DOWNLOAD grant — a folder grant must not
    // authorize arbitrary network egress.
    return [Capability.FILESYSTEM, Capability.DOWNLOAD];
  }
...
export function hostForCapability(capability, args, currentUrlOrHost, toolName) {
  ...
  if (capability === Capability.FILESYSTEM) {
    // Synthetic, human-readable grant key: the working directory itself.
    // Reads as "WebBrain wants to modify files and folders in the working
    // directory" in the permission prompt.
