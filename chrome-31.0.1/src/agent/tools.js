...
export const AGENT_TOOLS = [
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Download a resource (image, video, audio, file) from the current page by selector. Reads src/href/currentSrc/data-src. Handles blob: URLs (custom viewers, in-browser uploads). The file is saved to the user\'s configured download directory (default: the browser Downloads folder).',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      name: 'workspace_list',
      description: 'List files and folders inside the working directory (or a relative subfolder of it). Returns names, kinds (file/directory), and file sizes, sorted with folders first. Read-only.',
      parameters: {
        type: 'object',
        properties: {
          path: { type: 'string', description: 'Relative path inside the working directory. Empty or omitted = the directory root.' },
        },
      },
    },
  },
  {
    ...
    function: {
      ...
      description: 'Create a folder inside the working directory. Intermediate folders are created automatically, so "a/b/c" creates all three levels. Paths are always relative to the working directory — absolute paths and ".." are rejected.',
      parameters: {
        type: 'object',
        properties: {
          path: { type: 'string', description: 'Relative folder path to create, e.g. "reports/2026-08" or "images/batch-1"' },
        },
        required: ['path'],
      },
    },
  },
  {
    ...
    function: {
      ...
      description: 'Delete a file or folder inside the working directory (folders are deleted recursively). Refuses to delete the working directory root itself. Only delete items the user explicitly asked to remove — never delete anything unprompted.',
      parameters: {
        type: 'object',
        properties: {
          path: { type: 'string', description: 'Relative path of the file or folder to delete inside the working directory' },
        },
        required: ['path'],
      },
    },
  },
  {
    ...
    function: {
      ...
      description: 'Create a text file inside the working directory with content you generate — reports, CSV/JSON data, markdown, HTML, code, notes, etc. Intermediate folders are created automatically. Duplicate names get a -N suffix instead of overwriting unless overwrite:true is passed. Absolute paths and ".." are rejected.',
      parameters: {
        type: 'object',
        properties: {
          path: { type: 'string', description: 'Relative file path inside the working directory, e.g. "reports/summary.md"' },
          content: { type: 'string', description: 'The full text content to write' },
          overwrite: { type: 'boolean', description: 'Set true to overwrite an existing file of the same name (default false: a -N suffix is added)' },
        },
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Read a text file from the working directory (up to ~64K characters, then truncated). Binary files are refused. Use this to verify generated files or to pick up where a previous session left off. Read-only.',
      parameters: {
        type: 'object',
        properties: {
          path: { type: 'string', description: 'Relative file path inside the working directory' },
        },
        required: ['path'],
      },
    },
  },
  {
    ...
    function: {
      ...
      description: 'Download one or more files (images, documents, media) directly INTO the working directory — no save dialog, no Downloads-folder routing. Pass a single url or an array of urls (max 3 concurrent, max 50 total) and an optional subfolder that is created on demand. Filenames are taken from each URL and deduplicated with -N suffixes. Use this instead of download_files when the result should land inside the working directory.',
      parameters: {
        type: 'object',
        properties: {
          url: { type: 'string', description: 'Single URL to download into the working directory' },
          urls: { type: 'array', items: { type: 'string' }, description: 'Array of URLs to download into the working directory' },
          subfolder: { type: 'string', description: 'Optional relative subfolder inside the working directory (created automatically), e.g. "images/batch-1"' },
        },
      },
    },
  },
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Extract an archive that is inside the working directory. path = the archive\'s path relative to the working directory; destPath = where to extract (default: the archive\'s own folder — prefer passing the subfolder you created for this task so loose files stay contained). Supported formats: zip, 7z, rar, tar, gz, xz, bz2 and other 7-Zip formats. Internal folder structure is preserved, entry names are sanitized, existing files are never overwritten (-N suffix), and unsafe/oversized entries are skipped with reasons. Password-protected archives cannot be unpacked and report no output.',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description: 'One-shot media downloader for the current page. Downloads the main photo/video on single-content pages, or every media item on feeds when scroll:true. Handles common site DOM quirks, upgrades to max resolution, pairs DASH video+audio, stitches HLS (incl. AES-128 encrypted), and falls back to opening in a new tab when a CDN blocks CORS. PREFER this over manual DOM inspection plus generic file/resource-download workflows whenever the user asks to "download this image/video", "save this photo", or "grab the media" — it is a single call instead of figuring DOM selectors out manually. Do not ask which site this is or whether the page is "supported" — try it and report the actual result. Files land in the user\'s configured download directory (default: the browser Downloads folder); call list_downloads afterwards to confirm. RESULT SHAPE: `count` is total URLs found; `triggeredCount` is how many we tried to download; `completedCount` is how many were successfully fetched-and-saved; `openedInTabCount` are URLs the browser blocked from direct fetch (we opened them in a new tab — popup-blocking usually kills these AFTER the first one, so a count > 1 here means most did NOT actually save); `failedCount` are hard errors. ALWAYS report honestly: if `completedCount` is much smaller than `count`, say so — do not claim "downloads in progress in the background"; the run is fully synchronous and what is not in `completedCount` is not coming. May also include a `recommendation` object ({kind, message}) when the in-browser path cannot fully handle the request (DRM video, MSE blob with nothing buffered yet, a page with no downloadable media, empty result). When present, relay `recommendation.message` verbatim to the user — it names the right external CLI tool (yt-dlp or gallery-dl) and includes a copy-pasteable command.',
      parameters: {
        type: 'object',
        properties: {
          strategy: {
