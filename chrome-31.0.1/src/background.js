...
// Persona: user-defined assistant identity appended to the system prompt
// right after the base prompt. Seeded once from the bundled persona.js at
// the extension root; afterwards the settings UI owns the value. Empty
// string = user cleared it deliberately — do not re-seed.
async function loadPersona() {
  const stored = await chrome.storage.local.get(['personaText']);
  if (stored.personaText === undefined && PERSONA && PERSONA.trim()) {
    agent.personaText = PERSONA.trim();
    chrome.storage.local.set({ personaText: agent.personaText }).catch(() => {});
    return;
  }
...
// (workspace-fs.js — handles are structured-cloneable, not JSON); the
...
chrome.storage.onChanged.addListener((changes) => {
  ...
  if (changes.personaText) {
    agent.personaText = changes.personaText.newValue || '';
    refreshPrompts = true;
  }
...
async function handleMessage(msg, sender) {
  ...
  switch (msg.action) {
    ...
    case 'get_working_directory_state': {
      const stored = await chrome.storage.local.get(['workingDirectory']);
      const state = stored.workingDirectory || null;
      let permission = 'none';
      if (state) {
        try {
      ...
      return { ok: true, workingDirectory: state, permission };
    }
    ...
    case 'clear_working_directory': {
      await clearWorkspaceHandle();
      await chrome.storage.local.remove('workingDirectory');
