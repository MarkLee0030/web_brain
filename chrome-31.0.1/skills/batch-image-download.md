...
- **Archives**: when a page offers a single .zip (or the virtual-click download turns out to be one), the right sequence is: (1) copy/move the archive into the `<page-title>` subfolder of the working directory, (2) `workspace_extract({ path: '<page-title>/<file>.zip' })` to unpack it there — a zip that contains loose images without a wrapping folder stays contained in the page-title folder. `workspace_extract` supports ZIP only; for .rar/.7z tell the user plainly that the format is not supported and ask them to extract it manually (then the files can be copied into the working directory via the whitelist).
