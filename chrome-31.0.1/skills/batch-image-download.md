...
## Blocker sites, whitelisted folders, and archives

- **Blocker sites** (anti-hotlink/CDN/CORS walls that make `workspace_download`'s fetch fail with 403/404): the fallback is a virtual click on the page's own download button. That download lands in Chrome's DEFAULT Downloads folder, NOT the working directory. After the download finishes (confirm with `list_downloads`), move it in with `workspace_copy_in({ dir: '<whitelisted folder name>', path: '<filename>', destPath: '<page-title>/<filename>' })` — the user adds the Downloads folder once in Settings → 工作目录白名单, and `workspace_whitelist_list` shows which folders are available. Do not claim the file is lost or out of reach: the whitelist exists exactly for this case.
- **Archives**: when a page offers a single archive (zip/7z/rar/tar/gz/xz/bz2) or the virtual-click download turns out to be one, the right sequence is: (1) copy/move the archive into the `<page-title>` subfolder of the working directory, (2) `workspace_extract({ path: '<page-title>/<file>.zip' })` to unpack it there — an archive that contains loose images without a wrapping folder stays contained in the page-title folder. `workspace_extract` handles zip, 7z, rar, tar, gz, xz and bz2 via the bundled 7-Zip engine. Password-protected archives cannot be unpacked — if the engine reports no output, tell the user the archive is encrypted and ask for the password or a manual extraction.
