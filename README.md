...
- **历史对话续接**：历史页每条记录带「继续对话」按钮——把该对话恢复到侧栏并保持为活跃会话（沿用原 conversationId，后续消息继续追加到同一条记录，不会分叉）。agent 对话上下文按 conversationId 镜像进 IndexedDB（浏览器重启后 storage.session 丢失也能续上；本功能上线前保存的旧记录只有可见聊天，agent 上下文为空）。
- **Markdown 表格渲染**：聊天输出自动把 GFM 管道表格渲染为真实表格（支持左/中/右对齐、单元格内行内代码/加粗/斜体/链接、转义管道、溢出横向滚动）。
- **技能写作元技能**：对模型说「给这个网站写个技能」，它按 webbrain-skill 标准格式输出完整技能文档（含 summary/modes/intents 元数据块 + 正文，且遵循本 fork 的工作目录/批量下载/白名单/解压约定），你复制后粘贴到 设置 → Skills 即可。配套把消息复制按钮改为复制**原始 markdown**（渲染文本会丢失代码围栏，粘贴会坏格式）。
